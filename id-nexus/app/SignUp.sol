...
contract Test {
    ...
    // Set aadharnumber
    function setAadharNumber(string memory _aadharNumber) public {
        userIdentity[msg.sender].aadharNumber = _aadharNumber;
        emit IdentityUpdated(msg.sender, "aadharNumber", _aadharNumber);
    }

    // Set Pancard number
    function setPanCardNumber(string memory _panCardNumber) public {
        userIdentity[msg.sender].panCardNumber = _panCardNumber;
        emit IdentityUpdated(msg.sender, "panCardNumber", _panCardNumber);
    }
    ...
    function getIdentity(address _user) public view returns (string memory name, string memory phoneNumber,
     string memory _address, string memory email, string memory aadharNumber, 
     string memory panCardNumber, bool verified) 
     {
        Identity memory identity = userIdentity[_user];
        return (identity.name, identity.phoneNumber, identity.home_address, identity.email, identity.aadharNumber, identity.panCardNumber, identity.verified);
    }
}
